...
namespace EmpyrionScripting.DataWrapper
{
    public class StructureTank : IStructureTankWrapper
    {
        ...
        {
            return EmpyrionScripting.Configuration.Current.StructureTank[type].Any(I => I.ItemId == itemId);
        }

        public int ItemsNeededForFill(int itemId, int maxLimit)
        {
            return Math.Max(0, (int)((Capacity * (maxLimit / 100f)) - Content) / EmpyrionScripting.Configuration.Current.StructureTank[type].First(I => I.ItemId == itemId).Amount);
        }
    }
}